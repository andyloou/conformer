...
class ConformerFeedForward(nn.Module):
    def __init__(self, d_model, d_ff, dropout, use_bias=True):
        ...
        self.activation = nn.ReLU()  # or Swish()
...
class ConformerConvolution(nn.Module):
    ...
    def forward(self, x, pad_mask=None, cache=None):
        ...
        # Reshape thay vì transpose
        x = x.view(batch_size * seq_len, channels).unsqueeze(0)
        ...
        if isinstance(self.batch_norm, nn.LayerNorm):
            # Reshape về (batch, seq, channels) cho LayerNorm
            x = x.view(batch_size, channels, seq_len).view(batch_size, seq_len, channels)
            x = self.batch_norm(x)
            x = x.view(batch_size, channels, seq_len)
        else:
            x = self.batch_norm(x)
...
class ConformerLayer(nn.Module):
    ...
    def forward(self, x, att_mask=None, pos_emb=None, pad_mask=None, cache_last_channel=None, cache_last_time=None):
        # Feed forward 1
        ff1_out = self.dropout(self.feed_forward1(self.norm_feed_forward1(x)))
        x = x + ff1_out  # Thay vì dùng fc_factor
        
        # Self attention
        att_out = self.dropout(self.self_attn(query=self.norm_self_att(x), key=self.norm_self_att(x), 
                                            value=self.norm_self_att(x), mask=att_mask, pos_emb=pos_emb))
        x = x + att_out
        
        # Convolution
        conv_out = self.dropout(self.conv(self.norm_conv(x), pad_mask=pad_mask))
        x = x + conv_out
        
        # Feed forward 2
        ff2_out = self.dropout(self.feed_forward2(self.norm_feed_forward2(x)))
        x = x + ff2_out
...
class ConformerEncoder(nn.Module):
    ...
    def forward(self, audio_signal, length):
        ...
        if audio_signal.dim() == 3 and audio_signal.size(1) == self._feat_in:
...
class ConvASRDecoder(nn.Module):
    def __init__(self, feat_in=176, num_classes=1024, init_mode="xavier_uniform", vocabulary=None, add_blank=True):
        ...
        # Apply Conv1d on time dimension instead of feature dimension
    ...
    def forward(self, encoder_output):
        # encoder_output: [batch, feat_in, time]
        logits = self.decoder_layers(encoder_output)  # [batch, num_classes, time]
        if self.temperature != 1.0:
            logits = logits / self.temperature
        # Apply log_softmax on the num_classes dimension (dim=1)
